class LevelGen {
  constructor(heroSize) {
    ...
    this.squares = new Array(this.rows * this.cols).fill().map((_, i) => ({
      x: i % this.cols,
    ...
    this.array2D = this.convertTo2DArray();
  }
  ...
  convertTo2DArray() {
    ...
    for (let i = 0; i < this.rows; i++) {
      ...
      for (let j = 0; j < this.cols; j++) {
        array2D[i][j] = this.squares[i * this.cols + j];
      }
    }
    return array2D;
  }
  ...
  drawSquares() {
    ...
    let debugCanvas = this.debugCanvas;
    let debugCtx = this.debugCtx;
    let squares = this.squares;
    ...
    for (let i = 0; i < squares.length; i++) {
      ...
      if (!squares[i].isSolid) {
        ...
        debugCtx.strokeStyle = "rgba(255, 255, 255, 1)";
        debugCtx.strokeRect(x, y, size, size);
      } else {
    ...
    // loop through edge cells
    for (let i = 0; i < this.edgeCells.length; i++) {
      // draw edge cell
      const x = this.edgeCells[i].x;
      const y = this.edgeCells[i].y;

      debugCtx.fillStyle = "rgba(255, 255, 255, 1)";
      debugCtx.fillRect(x, y, size, size);
    }
  }
